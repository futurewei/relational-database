...
public class IntHistogram implements Histogram<Integer>{
  ...
  public float computeReductionFactor(PredicateOperator predicate,
    ...
    int predValue = value.getInt();
    int bucketMin = getMinValue();
    int bucketMax = getMaxValue();
    int numDistinct= getNumDistinct();
    switch(predicate){
          case EQUALS:
            reductionFactor=(float) 1/numDistinct;
            break;
          case LESS_THAN:
              reductionFactor= (float) (predValue-bucketMin) / (bucketMax-bucketMin);
              break;
          case LESS_THAN_EQUALS:
              reductionFactor= (float) 1/numDistinct+(float)(predValue-bucketMin)/(bucketMax-bucketMin);
              break;
          case GREATER_THAN:
             reductionFactor= (float)(bucketMax-predValue)/(bucketMax-bucketMin);
             break;
          case GREATER_THAN_EQUALS:
            reductionFactor = (float) 1/numDistinct + (float)(bucketMax - predValue)/(bucketMax - bucketMin); //???not sure
            break;
          default:
          break;
    }
